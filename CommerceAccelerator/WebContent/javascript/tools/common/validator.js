...
function isValidEmail(strEmail){
  ...
  var localpart = strEmail.substring(0,strEmail.lastIndexOf("@"));
  if(strEmail.length == 0) {
    return true;
  } else if (strEmail.length < 5 || localpart.length > 64) {
             return false;
    } else {
      if (strEmail.match(/"\s+"/)){
        return false;
              ...
              if (strEmail.lastIndexOf(".") < (strEmail.indexOf("@") + 2)){
                return false;
                    ...
                    if (strEmail.lastIndexOf(".") >= strEmail.length-1){
                      ...
                      if(strEmail.match(/\.{2,}/) != null){
                        return false;
                      }else{
                        if(localpart.indexOf("\"") > -1){
                          if(localpart.match(/"/g).length%2==1){
                            return false;
                          }else{                            
                            if(localpart.match(/^"[^"]*"$|\."[^"]*"\.|\."[^"]*"$|^"[^"]*"\./) == null
                                || localpart.match(/^"[^"]*"$|\."[^"]*"\.|\."[^"]*"$|^"[^"]*"\./).length != localpart.match(/"/g).length/2){
                              return false;
                            }
                          }
                        }
                        if(localpart.indexOf("@") > -1){
                          if(localpart.match(/^[^"]*@/) != null || localpart.match(/@[^"]*$/) != null){
                            return false;
                          }
                        }
                      }
