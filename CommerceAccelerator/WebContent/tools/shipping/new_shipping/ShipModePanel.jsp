...
<%
	String readOnly = request.getParameter(ShippingConstants.PARAMETER_READONLY);
	  ...
	  ShippingModeDataBean shipModeBean = new ShippingModeDataBean();
	  if (!newShipMode && shipModeId != null && !shipModeId.equals("")) {
		    shipModeBean.setInitKey_shippingModeId(shipModeId);
		    DataBeanManager.activate(shipModeBean);
	  }
...
<SCRIPT>
document.writeln('<i>'+'<%=UIUtil.toHTML(shipModeBean.getCarrier())%>'+'</i>');
</SCRIPT>
<p><%= shippingRB.get(ShippingConstants.MSG_SHIPPING_SERVICE_PROMPT) %><br/>
<SCRIPT>
document.writeln('<i>'+'<%=UIUtil.toHTML(shipModeBean.getCode())%>'+'</i>');
</SCRIPT>
<%
       } else { 
%> 
