...
<c:if test="${!(empty onlineStore) && !(empty onlineStore.userData)}">
	<c:forEach var="userDataField" items="${onlineStore.userData.userDataField}">
		...
		<c:if test="${userDataField.typedKey == 'headlessStore'}">
			<c:set var="headlessStore" value="${userDataField.typedValue}" />
		</c:if>
	</c:forEach>
</c:if>
<values>
<%-- cmsPunchOut and ExternalContent cannot be enabled at same time --%>
	...
	<c:choose>
		<c:when test="${headlessStore == 'TRUE'}">
			<headlessStore>true</headlessStore>
		</c:when>
		<c:otherwise>
			<headlessStore>false</headlessStore>
		</c:otherwise>
	</c:choose>
</values>
