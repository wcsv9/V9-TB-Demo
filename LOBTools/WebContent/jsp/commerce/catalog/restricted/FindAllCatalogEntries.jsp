...
<c:choose>
	...
	<c:when test="${(empty param.searchText) && (param.typeProducts == 'false') && (param.typeSKUs == 'false') && (param.typeVariants == 'false')
					&& (empty param.searchSource)
		...
		<c:choose>
			...
			<c:otherwise>
				...
				<wcf:getData type="com.ibm.commerce.catalog.facade.datatypes.CatalogEntryType[]"
					...
					<c:set var="variantExp" value="VariantBean"/>
					<c:set var="pdkExp" value="PredDynaKitBean"/>
						...
						<c:if test="${(empty param.catentryTypes)}">
								<wcf:param name="catEntryTypes" value="${productExp},${bundleExp},${kitExp},${dynamicKitExp},${pdkExp},${SKUExp},${variantExp}"/>
						</c:if>
						...
						<c:if test="${param.catentryTypes == '2'}">
							...
							<c:if test="${param.typeVariants == 'true'}">
								<c:if test="${typeParam != ''}" >
									<c:set var="typeParam" value="${typeParam},"/>
								</c:if>
								<c:set var="typeParam" value="${typeParam}${variantExp}"/>
							</c:if>
