							...
							if(catalogAttribute.getAttributeIdentifier()!=null && catalogAttribute.getAttributeIdentifier().getUniqueID()!=null)
							{
								...
								if(attributeID != null)
								{
									...
									if(!PROCESSED_ATTRIBUTES.contains(attributeID)) {
										...
										<c:if test="${ catentry.catalogEntryTypeCode == 'VariantBean' }">
											<jsp:directive.include file="serialize/SerializeVariantDefiningAttributeDictionaryAttributes.jspf"/>
										</c:if>
										<c:if test="${ catentry.catalogEntryTypeCode != 'ItemBean' && catentry.catalogEntryTypeCode != 'VariantBean'}">
