...
<%-- Get the reference parent product, for a variant --%>
<objects>
	<c:if test="${(catalogEntry != null) && !(empty catalogEntry.parentCatalogEntryIdentifier)}">
		...
		<c:set var="referenceObjectType" value="ProductChildVariant"/>
		<c:set var="referenceObjectTypeLocked" value='readonly="true"'/>
		<c:if test="${param.objectStoreId != param.storeId }">
			<c:set var="referenceObjectType" value="ProductChildInheritedVariant"/>
		</c:if>
