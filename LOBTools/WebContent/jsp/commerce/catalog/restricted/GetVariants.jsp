...
<wcf:getData
	...
	var="catalogEntries" expressionBuilder="getCatalogEntryByParentCatalogEntryAndCatalogEntryRelationTypeWithoutADAllowedValuesId"
	...
	<wcf:param name="catRelTypeID" value="PRODUCT_VARIANT" />
	<wcf:param name="dataLanguageIds" value="${param.defaultLanguageId}"/>
	<wcf:param name="accessProfile" value="IBM_Admin_CatalogEntrySKUsWithoutADAllowedValues"/>
</wcf:getData>
	...
	<c:if test="${!(empty catalogEntries)}">
		<c:forEach var="catalogEntry" items="${catalogEntries}">
			...
			<c:choose>
				<c:when test="${catalogEntry.catalogEntryTypeCode == 'VariantBean'}">
					<c:if test="${catalogEntry.catalogEntryIdentifier.externalIdentifier.storeIdentifier.uniqueID == param.storeId }">
						<c:set var="objectType" value="Variant"/>
						<c:set var="childType" value="ProductChildVariant"/>
					</c:if>
					<c:if test="${catalogEntry.catalogEntryIdentifier.externalIdentifier.storeIdentifier.uniqueID != param.storeId }">
						<c:set var="objectType" value="InheritedVariant"/>
						<c:set var="childType" value="ProductChildInheritedVariant"/>
			...
			<object objectType="${childType}" ${childTypeLocked}>
				...
				<jsp:directive.include file="serialize/SerializeVariant.jspf"/>
			</object>
		</c:forEach>
	</c:if>
</objects>