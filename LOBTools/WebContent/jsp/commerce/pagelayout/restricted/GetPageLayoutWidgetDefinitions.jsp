...
<wcf:getData type="com.ibm.commerce.infrastructure.facade.datatypes.ConfigurationType"
	var="supportedLanguagesConfig" expressionBuilder="findByUniqueID">
	<wcf:contextData name="storeId" data="0" />
	<wcf:param name="uniqueId" value="com.ibm.commerce.foundation.supportedLanguages" />
</wcf:getData>
<c:set var="dataLanguageIds" value=""/>
<c:forEach var="attribute" items="${supportedLanguagesConfig.configurationAttribute}">
	<c:set var="currentLocale" value=""/>
	<c:set var="currentLanguageId" value=""/>
	<c:forEach var="additionalValue" items="${attribute.additionalValue}">
		<c:if test="${additionalValue.name == 'localeName'}">
			<c:set var="currentLocale" value="${additionalValue.value}"/>
		</c:if>
		<c:if test="${additionalValue.name == 'languageId'}">
			<c:set var="currentLanguageId" value="${additionalValue.value}"/>
		</c:if>
	</c:forEach>
	<c:if test="${currentLocale == param.locale}">
		<c:set var="dataLanguageIds" value="${currentLanguageId}"/>
	</c:if>
</c:forEach>
...
<wcf:getData type="com.ibm.commerce.pagelayout.facade.datatypes.WidgetDefinitionType[]"
	...
	<wcf:param name="dataLanguageIds" value="${dataLanguageIds}"/>
</wcf:getData>
