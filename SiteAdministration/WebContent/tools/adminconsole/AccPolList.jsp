...
function deleteRole()
{
	...
	var count = 0;
<% 	
	final String findIfAnyUserHasPolicyAccount =
				"SELECT T1.PLCYACCT_ID FROM PLCYACCT T1 WHERE (T1.PLCYACCT_ID = ?) AND EXISTS (SELECT 1 FROM USERREG T2 WHERE T2.PLCYACCT_ID = T1.PLCYACCT_ID)";
	...
	for (int i=0; i < accPolList.size(); i++) {
		...
		try {
			...
			Vector vecRows = abServerJDBCHelper.executeParameterizedQuery(findIfAnyUserHasPolicyAccount, parameters);	
