	...
	sql=
			...
			(
				...
				WHERE 
					...
					CATENTREL.CATRELTYPE_ID in ('PRODUCT_ITEM')
			)
			ORDER BY 
				SEQUENCE
	...
	sql=
			...
			SELECT 
	     	...
	     	FROM
     				CATENTREL
	     	WHERE
					CATENTREL.CATENTRY_ID_PARENT IN (?UniqueID?) AND
					CATENTREL.CATRELTYPE_ID NOT IN ('PRODUCT_VARIANT')
			ORDER BY
					CATENTREL.SEQUENCE						
...
<!-- ============================================================= -->
<!-- This SQL will return the elements of the Catalog Entry        -->
<!-- noun(s) which are the specified catalog entry type children   -->
<!-- of the given catalog entry (unique identifier)                -->
<!-- Multiple results are returned if multiple identifiers are     -->
<!-- specified.	                                                   -->
<!-- The access profiles that apply to this SQL are:               -->
<!-- IBM_Admin_Details, IBM_Admin_Summary, IBM_Admin_All and all   -->
<!-- other catalog entry profiles.		                           -->
<!-- @param UniqueID - The identifier(s) for which to retrieve.    -->
<!-- @param CatRelTypeID - The child catentry and parent catentry's-->
<!-- relationship  type.                     	                   -->
<!-- ============================================================= -->
BEGIN_XPATH_TO_SQL_STATEMENT
	name=/CatalogEntry[CatalogEntryRelationTypeId= and ParentCatalogEntryIdentifier[(UniqueID=)]]
	sql_key_processor=com.ibm.commerce.catalog.facade.server.services.dataaccess.processor.CatalogEntryFilterKeyProcessor
	base_table=CATENTRY
	sql=
			...
			SELECT 
	     	...
	     	WHERE
					CATENTREL.CATENTRY_ID_PARENT IN (?UniqueID?) AND
					CATENTREL.CATRELTYPE_ID IN (?CatalogEntryRelationTypeId?)
			ORDER BY
					CATENTREL.SEQUENCE						
...
<!-- This associated SQL:                                      -->
<!-- Adds Base Catentry Info 				                   -->
<!-- to the resultant data graph for catalog filter.           -->
<!-- ========================================================= -->
BEGIN_ASSOCIATION_SQL_STATEMENT
	name=IBM_RootCatalogEntryForCatalogFilter
	base_table=CATENTRY
	additional_entity_objects=true
	sql=
		SELECT 
				...
				CATENTDESC.$COLS:CATENTDESC:NAME$,STORECENT.$COLS:STORECENT:CATENTRY_ID$,
				STORECENT.$COLS:STORECENT:STOREENT_ID$
		...
		FROM
				...
				CATENTDESC ON 
				 ...
				 LEFT OUTER JOIN
				STORECENT ON 
					(STORECENT.CATENTRY_ID = CATENTRY.CATENTRY_ID 	AND 
					 STORECENT.STOREENT_ID IN ($STOREPATH:catalog$))
		WHERE
               CATENTRY.CATENTRY_ID IN ($ENTITY_PKS$)
...
<!-- ============================================================== -->
<!-- This associated SQL:                                           -->
<!-- Adds parent info of a SKU or a PDK (PRODUCT_ITEM or DK_PDK or PRODUCT_VARIANT relationship)  -->
<!-- to the resultant data graph.                                   -->
<!-- ============================================================== -->
BEGIN_ASSOCIATION_SQL_STATEMENT
	...
	sql=
		...
		WHERE
				...
				CATENTREL.CATRELTYPE_ID IN( 'PRODUCT_ITEM','DK_PDK','PRODUCT_VARIANT','VARIANT_ITEM')
...
<!-- This profile returns the following info:                  -->
<!-- 	1) Catalog Entry with description summary.             -->
<!-- 	2) Catalog Entry parent catalog entry.                 -->
<!-- ========================================================= -->
BEGIN_PROFILE
  name=IBM_Admin_Minimum
  BEGIN_ENTITY
    base_table=CATENTRY
    className=com.ibm.commerce.catalog.facade.server.services.dataaccess.graphbuilderservice.CatalogEntryGraphComposer
    associated_sql_statement=IBM_RootCatalogEntryForCatalogFilter
  END_ENTITY
END_PROFILE
