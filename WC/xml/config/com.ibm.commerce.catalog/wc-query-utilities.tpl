...
BEGIN_SQL_STATEMENT
  name=GET_CATENTRY_BY_STORE
	base_table=STORECENT
  sql=SELECT a.CATENTRY_ID,a.STOREENT_ID FROM STORECENT a, CATENTRY b WHERE a.STOREENT_ID IN (?storeId?) AND a.CATENTRY_ID=b.CATENTRY_ID AND b.CATENTTYPE_ID IN ('ProductBean','ItemBean','PackageBean','VariantBean') AND b.MARKFORDELETE = 0 order by a.catentry_id
END_SQL_STATEMENT
...
BEGIN_SQL_STATEMENT
  ...
  FROM (SELECT ROW_NUMBER() OVER(ORDER BY q.CATENTRY_ID) AS rn, 
        q.* 
        from 
  			(select distinct CATENTRY.CATENTRY_ID, CATENTRY.CATENTTYPE_ID FROM CATENTRY, STORECENT WHERE CATENTRY.MARKFORDELETE=0  AND CATENTRY.CATENTTYPE_ID IN('ProductBean','ItemBean','PackageBean','VariantBean') AND  CATENTRY.CATENTRY_ID=STORECENT.CATENTRY_ID AND STORECENT.STOREENT_ID=?storeId?) q
  		)
  ...
  dbtype=derby
  sql=SELECT a.CATENTRY_ID,a.STOREENT_ID FROM STORECENT a, CATENTRY b WHERE a.STOREENT_ID IN (?storeId?) AND a.CATENTRY_ID=b.CATENTRY_ID AND b.CATENTTYPE_ID IN ('ProductBean','ItemBean','PackageBean','VariantBean') AND b.MARKFORDELETE = 0 and a.catentry_id between  ?start_value? AND ?end_value?  order by a.catentry_id
...
<!-- =============================================================================== 
	Create a new catalog group to catalog entry relations for Rules Based Sales Category
	@param catGroupId The catalog group id	
	@param catalogId The catalog id for the new relations
	@param catEntryId The catalog entry id 	
	@param sequence The display sequence number	
==================================================================================== -->
BEGIN_SQL_STATEMENT
	name=IBM_Insert_CreateCatalogGroupToCatalogEntryRelationForRuleBasedCategories
	base_table=CATGPENREL
	dbtype=oracle
	sql=
	INSERT INTO CATGPENREL (CATGROUP_ID, CATALOG_ID, CATENTRY_ID, SEQUENCE, LASTUPDATE)
	select TO_NUMBER(?catGroupId?), TO_NUMBER(?catalogId?), t1.catentry_id, TO_NUMBER(?sequence?), SYSTIMESTAMP
	from catentry t1 where t1.catentry_id=?catEntryId? and 
	not exists (select 1 
	   from catgpenrel t2
	   where t2.catgroup_id=?catGroupId? and t2.catalog_id=?catalogId? and t2.catentry_id=t1.catentry_id)

	dbtype=db2
	sql=
	INSERT INTO CATGPENREL (CATGROUP_ID, CATALOG_ID, CATENTRY_ID, SEQUENCE, LASTUPDATE)
	select CAST(?catGroupId? as BIGINT), CAST(?catalogId? as BIGINT), t1.catentry_id, CAST(?sequence? as DOUBLE), current timestamp
	from catentry t1
	where t1.catentry_id = ?catEntryId?
	except 
		select t2.catgroup_id, CAST(?catalogId? as BIGINT), t2.catentry_id, t2.sequence, current timestamp 
		from catgpenrel t2 
		where t2.catgroup_id=?catGroupId? and t2.catalog_id=?catalogId?
		 	
	sql=
	INSERT INTO CATGPENREL (CATGROUP_ID, CATALOG_ID, CATENTRY_ID, SEQUENCE, LASTUPDATE)
	select CAST(?catGroupId? as BIGINT), CAST(?catalogId? as BIGINT), t1.catentry_id, CAST(?sequence? as DOUBLE), current timestamp
	from catentry t1
	where t1.catentry_id = ?catEntryId? and 
	not exists (select 1 
	   from catgpenrel t2
	   where t2.catgroup_id=?catGroupId? and t2.catalog_id=?catalogId? and t2.catentry_id=t1.catentry_id)
...
BEGIN_SQL_STATEMENT
	...
	from catentrel t1
	where t1.catentry_id_parent=?catEntryId? and t1.CATRELTYPE_ID='PRODUCT_ITEM' and
	not exists ( select 1 from catgpenrel t2 where t2.catgroup_id=?catGroupId? and t2.catalog_id=?catalogId? 
	and t1.catentry_id_child=t2.catentry_id ) 
...
END_SQL_STATEMENT
<!-- ========================================================================= -->
<!--  To get enabled features list by storeid                                  -->
<!-- ========================================================================= -->
BEGIN_SQL_STATEMENT
	name=QUERY_LIST_OF_ENABLED_FEATURES
    base_table=EMSPOT
	sql=
    SELECT T1.NAME FROM EMSPOT T1 
	WHERE T1.EMSPOT_ID IN (SELECT T2.EMSPOT_ID FROM DMEMSPOTDEF T2 WHERE T2.CONTENT='true' 
	AND T2.EMSPOT_ID IN (SELECT T3.EMSPOT_ID FROM EMSPOT T3  WHERE USAGETYPE='STOREFEATURE') 
	AND T2.STOREENT_ID IN (?storeId?) 
	AND T2.CONTENTTYPE='FeatureEnabled')
END_SQL_STATEMENT