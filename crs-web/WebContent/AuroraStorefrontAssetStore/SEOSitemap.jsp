		...
		<c:if test="${urlCounter >= beginIndex && urlCounter < maxUrlsToGenerate}">
			...
			<c:if test="${replaceHost eq 'true'}">
 			...
 			<c:if test="${replacePort eq 'true'}">			
				<%
 				String url = pageContext.getAttribute("TopCategoriesDisplayURL").toString();				
 				String portNew = pageContext.findAttribute("portNumber").toString(); 								
 				String[] urlSplit = url.split("/");
 				String protocol = urlSplit[0];
 				String[] hostAndPort = urlSplit[2].split(":");
 				String urlLeft = protocol+"//"+hostAndPort[0];
 				String urlRight = "";
 				if(hostAndPort.length==2)
 					urlRight = url.substring(urlLeft.length()+hostAndPort[1].length()+1);
 				else
 					urlRight = url.substring(urlLeft.length()+1);
 				if((protocol.equals("http:")&&portNew.equals("80"))||(protocol.equals("https:")&&portNew.equals("443")))
 					url = urlLeft+urlRight;
 				else
 					url = urlLeft+":"+portNew+urlRight;
 				pageContext.setAttribute("TopCategoriesDisplayURL",url);
 				 %>
 			</c:if>
    ...
    ***
	...
	<c:forEach var="dbLanguage" items="${languageIds}" varStatus="status">
		...
		<c:if test="${urlCounter >= beginIndex && urlCounter < maxUrlsToGenerate}">
            ...
            <c:if test="${replacePort eq 'true'}">			
				<%
 				String url = pageContext.getAttribute("pageViewURL").toString();				
 				String portNew = pageContext.findAttribute("portNumber").toString(); 								
 				String[] urlSplit = url.split("/");
 				String protocol = urlSplit[0];
 				String[] hostAndPort = urlSplit[2].split(":");
 				...
 				if((protocol.equals("http:")&&portNew.equals("80"))||(protocol.equals("https:")&&portNew.equals("443")))
 					...
 					url = urlLeft+":"+portNew+urlRight;
 				pageContext.setAttribute("pageViewURL",url);
 				 %>
 			</c:if>
        ...
        <c:forEach items="${landingPages.resultList}" var="curLandingPage">
			...
			<c:forEach var="searchTerm" items="${searchTermArray}" varStatus="count">
				...
				<c:if test="${urlCounter >= beginIndex && urlCounter < maxUrlsToGenerate}">
					...
					<c:if test="${replacePort eq 'true'}">			
						<%
		 				String url = pageContext.getAttribute("landingPageURL").toString();				
		 				String portNew = pageContext.findAttribute("portNumber").toString(); 								
		 				String[] urlSplit = url.split("/");
		 				String protocol = urlSplit[0];
		 				String[] hostAndPort = urlSplit[2].split(":");
		 				...
		 				if((protocol.equals("http:")&&portNew.equals("80"))||(protocol.equals("https:")&&portNew.equals("443")))
		 					...
		 					url = urlLeft+":"+portNew+urlRight;
		 				pageContext.setAttribute("landingPageURL",url);
		 				 %>
		 			</c:if>
