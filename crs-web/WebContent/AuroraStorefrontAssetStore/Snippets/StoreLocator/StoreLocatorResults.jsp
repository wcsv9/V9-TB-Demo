		...
		<table id="bopis_table" tabindex="-1" summary="<fmt:message bundle="${storeText}" key='STORE_RESULTS_SUMMARY'/>" cellpadding="0" cellspacing="0" border="0" width="100%">
			...
			<c:forEach var="i" begin="0" end="${resultNum-1}">
				...
				<%-- 	<c:choose>
						...
						<c:choose>
							<c:when test="${storeExistInCookie == 'true'}">
								...
								<div id="addPhysicalStoreToCookie<c:out value='${physicalStores.PhysicalStore[i].uniqueID}' />" style="display:none;">
									<a href="#" role="button" class="button_primary" id="addPhysicalStoreToCookieButton<c:out value='${i}' />" onclick="Javascript:setCurrentId('addPhysicalStoreToCookieDisabled<c:out value='${physicalStores.PhysicalStore[i].uniqueID}' />'); if (storeLocatorJSStore.addPhysicalStore(<c:out value="${physicalStores.PhysicalStore[i].uniqueID}" />, <c:out value="${i}" />)) {storeLocatorJSStore.refreshStoreList('<c:out value="${fromPage}" />');} return false;" onBlur="JavaScript: this.blur();" onmouseout="JavaScript: this.blur();">
					...
					<%@ include file="StoreLocatorResults_body_ext.jspf"%>
